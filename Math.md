# 🧮 The Math Behind F1 Strategy Control
...
We approximate the lap time function $f(x)$ using **Random Forest Regression**. Unlike linear models, Random Forest captures the non-linear "cliff" in tire performance—where grip falls off drastically after a specific age.
...
1.  **Tire Age ($a$):** The number of laps driven on the current set.
    * *Math:* Degradation is modeled as a non-linear curve. $Performance \propto \frac{1}{e^{k \cdot a}}$ (Conceptually captured by the decision trees).
...
---
*Generated for the F1 Strategy Control Project.*
